...
async function createUserStory() {
  ...
  // Get story details
  const storyNumber =
    (await prompt(`Story number (default: ${nextNumber}): `)) || nextNumber
  ...
  const storyTitle = await prompt(
    'Story title (e.g., "Setup Leaflet and Create Basic Map"): '
  )
  if (!storyTitle) {
  ...
  const storyPoints =
    (await prompt('Story points (1/2/3/5/8, default: 2): ')) || '2'
  ...
  const templatePath = path.join(
    __dirname,
    '../templates/user-story-template/story.md'
  )
