...
function createGitBranch(epicNumber, epicName) {
  try {
    ...
    if (currentBranch !== 'master' && currentBranch !== 'main') {
      ...
      console.log(
        `   Please checkout master first if you want to branch from master.`
      )
    }
...
async function createEpic() {
  ...
  console.log(
    'This will create a new epic folder with template files and a git branch.\n'
  )

  // Get epic number
  const nextNumber = getNextEpicNumber()
  const epicNumber =
    (await prompt(`Epic number (default: ${nextNumber}): `)) || nextNumber
  ...
  // Get epic title
  const epicTitle = await prompt(
    'Epic title (e.g., "Interactive Map Implementation"): '
  )
  if (!epicTitle) {
  ...
  // Get other details
  const priority =
    (await prompt('Priority (High/Medium/Low, default: Medium): ')) || 'Medium'
  const storyPoints =
    (await prompt('Estimated story points (default: 5): ')) || '5'
  const targetWeek =
    (await prompt('Target completion (e.g., "Week 2", default: "TBD"): ')) ||
    'TBD'

  // Create git branch?
  const createBranch =
    (await prompt('Create git branch from master? (y/n, default: y): ')) || 'y'
  ...
  const templatePath = path.join(
    __dirname,
    '../templates/epic-template/epic.md'
  )
  ...
  template = template
    ...
    .replace(
      /epic\/XXX-epic-name/g,
      `epic/${paddedNumber}-${toKebabCase(epicName)}`
    )
